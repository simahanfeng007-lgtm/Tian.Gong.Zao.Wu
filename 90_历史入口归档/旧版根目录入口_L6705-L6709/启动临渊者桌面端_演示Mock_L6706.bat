...
cd /d "%~dp0"
echo 启动临渊者桌面端 L6.70.6：强制 mock 演示模式
call :find_python
if not defined LZ_PY goto no_python
echo 使用 Python：%LZ_PY%
%LZ_PY% desktop\start_linyuanzhe_desktop_l671.py --backend-mode mock
set "EXITCODE=%ERRORLEVEL%"
if not "%EXITCODE%"=="0" (
  echo.
  echo 启动失败，退出码：%EXITCODE%
  if exist reports\desktop_startup_failure_l6706.txt (
    echo.
    type reports\desktop_startup_failure_l6706.txt
  )
