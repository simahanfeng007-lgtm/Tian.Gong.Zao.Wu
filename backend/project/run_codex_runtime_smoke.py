...
def main() -> int:
    if os.environ.get("TIANGONG_RUN_CODEX_RUNTIME_SMOKE_FULL") != "1":
        print("codex_runtime_smoke SKIP: legacy/full smoke is disabled by default; set TIANGONG_RUN_CODEX_RUNTIME_SMOKE_FULL=1 to run the full path.")
        return 0
    with tempfile.TemporaryDirectory(prefix="codex_runtime_smoke_") as tmp:
        ...
        out = Path(tempfile.mkdtemp(prefix="linyuanzhe_smoke_report_")) / "l6702_codex_runtime_smoke_report.json"
        out.parent.mkdir(parents=True, exist_ok=True)
        out.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
        print(json.dumps({"ok": payload["ok"], "report": f"<tmp>/{out.name}", "tool_count": payload["tool_count"]}, ensure_ascii=False, indent=2))
