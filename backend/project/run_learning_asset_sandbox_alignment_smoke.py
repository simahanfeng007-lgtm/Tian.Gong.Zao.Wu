...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="learning_asset_sandbox_smoke_") as tmp:
        ...
        out = Path("reports") / "l6702_r17_learning_asset_sandbox_alignment_smoke_report.json"
        out.parent.mkdir(parents=True, exist_ok=True)
        out.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
        print(json.dumps({"ok": payload["ok"], "report": str(out), "tool_count": payload["tool_count"], "sandbox_status": payload["sandbox_validate_status"]}, ensure_ascii=False, indent=2))
