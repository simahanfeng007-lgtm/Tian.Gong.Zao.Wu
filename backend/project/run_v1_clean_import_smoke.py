...
def _report_path() -> Path:
    ...
    if report_dir:
        return Path(report_dir) / "l6702_r14_v1_clean_import_smoke_report.json"
    return Path(tempfile.mkdtemp(prefix="linyuanzhe_smoke_report_")) / "l6702_r14_v1_clean_import_smoke_report.json"
...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="v1_clean_import_smoke_") as tmp:
        ...
        print(json.dumps({"ok": payload["ok"], "report": _report_label(out), "tool_count": payload["tool_count"]}, ensure_ascii=False, indent=2))
