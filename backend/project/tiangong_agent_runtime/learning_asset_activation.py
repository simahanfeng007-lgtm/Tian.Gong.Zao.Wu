...
class LearningAssetActivationBridge:
    ...
    def apply(
        ...
        *,
        ...
        root = guard.resolve_for_artifact(ACTIVE_ROOT)
        ...
        _write_json(registry_path, {k: v for k, v in payload.items() if k not in {"activated_assets", "issues", "issue_count", "chain", "notes", "next_action_hint"}})
    ...
    def smoke(self, *, workspace: str | Path, sample_args: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        for record in status_report.get("records", []):
            ...
            try:
                ...
                call = _call_active_record(record, record_sample)
                results.append({
    ...
    def load_active_assets(self, *, workspace: str | Path, force: bool = False) -> dict[str, Any]:
        ...
        if not registry_path.exists():
            ...
            self._last_report = report
            return report
        if force or str(registry_path) not in self._loaded_registry_paths:
    ...
    def _activate_package(self, *, guard: WorkspaceGuard, root: Path, package: dict[str, Any], notes: str) -> dict[str, Any]:
        ...
        _write_json(active_dir / "activation_manifest.json", activation_manifest)
...
def _build_active_asset_adapter(record: dict[str, Any]):
    def adapter(invocation: ToolInvocation, context: TurnContext) -> ToolResult:
        try:
            payload = _call_active_record(record, invocation.arguments)
        except (OSError, ValueError, WorkspaceViolation) as exc:
            return ToolResult(
                invocation.step_id,
                invocation.tool_name,
...
def _call_active_record(record: dict[str, Any], arguments: dict[str, Any]) -> dict[str, Any]:
    active_dir = Path(str(record.get("active_dir") or "")).expanduser().resolve()
