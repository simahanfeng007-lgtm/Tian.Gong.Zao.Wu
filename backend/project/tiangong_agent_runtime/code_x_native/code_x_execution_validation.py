...
A5_BLOCK_TOKENS = {
    # L6.72.39：只保留极高危系统破坏类 token；普通 curl/wget/powershell/rm file
    # 不再被 token 级一刀切阻断，仍由 A5_DANGEROUS_PATTERNS 捕捉 rm -rf、format、mkfs 等。
    "format", "shutdown", "reboot", "halt", "mkfs", "diskpart", "dd",
}
...
def _classify_command_risk(args: Sequence[str]) -> Dict[str, Any]:
    ...
    # L6.72.39：依赖安装不再按默认规则阻断；若命令包含 A5 破坏性/提权模式，仍会被上方规则拦截。
