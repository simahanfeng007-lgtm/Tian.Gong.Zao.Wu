...
def _report_path() -> Path:
    ...
    if report_dir:
        return Path(report_dir) / "l6702_r15_runtime_tool_alignment_smoke_report.json"
    return Path(tempfile.mkdtemp(prefix="linyuanzhe_smoke_report_")) / "l6702_r15_runtime_tool_alignment_smoke_report.json"
...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="runtime_tool_alignment_smoke_") as tmp:
        ...
        print(json.dumps({"ok": payload["ok"], "report": _report_label(out), "tool_count": direct_payload["tool_count"], "usage_cards": direct_payload["usage_card_count"]}, ensure_ascii=False, indent=2))
