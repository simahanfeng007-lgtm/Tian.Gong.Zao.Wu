...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="learning_asset_contract_smoke_") as tmp:
        ...
        out = Path("reports") / "l6702_r16_learning_asset_contract_smoke_report.json"
        out.parent.mkdir(parents=True, exist_ok=True)
        out.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
        print(json.dumps({"ok": payload["ok"], "report": str(out), "tool_count": payload["tool_count"], "contract_status": payload["contract_validate_status"]}, ensure_ascii=False, indent=2))
