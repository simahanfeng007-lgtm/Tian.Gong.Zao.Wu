...
def test_l2_phase7_contains_no_real_io_network_model_tool_or_mutation_calls():
    violations = []
...
def test_l2_phase7_state_objects_remain_dataclass_facts_without_public_methods():
    violations = []
    for path in L2_DIR.glob("*.py"):
        if path.name not in PHASE7_FILES:
            continue
