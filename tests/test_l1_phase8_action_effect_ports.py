...
import tiangong_kernel.l1_ports.action_effect_ports as ports
...
def test_phase8_action_effect_ports_are_abstract():
...
def test_phase8_action_effect_methods_return_result_wrappers():
    for port_cls in PORT_CLASSES:
...
def test_phase8_action_effect_data_objects_are_frozen_slots_dataclasses():
    assert DATA_CLASSES
...
def test_phase8_action_effect_reuses_l0_and_l1_annotation_names():
    assert "ActionIntent" in ports.ActionIntentEnvelope.__annotations__["intent"]
    assert "EffectRef" in ports.EffectReport.__annotations__["effect_ref"]
    assert "TransactionRef" in ports.TransactionIntent.__annotations__["transaction_ref"]
    assert "DeletionRef" in ports.DeletionIntent.__annotations__["deletion_ref"]


def test_phase8_action_effect_ports_do_not_implement_real_capabilities_or_old_chain():
