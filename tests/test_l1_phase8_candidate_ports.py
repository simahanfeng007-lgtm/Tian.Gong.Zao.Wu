...
def test_phase8_candidate_ports_are_abstract():
...
def test_phase8_candidate_methods_return_result_wrappers():
    for port_cls in PORT_CLASSES:
...
def test_phase8_candidate_data_objects_are_frozen_slots_dataclasses():
    assert DATA_CLASSES
...
def test_phase8_candidate_reuses_l0_and_l1_annotation_names():
    assert "ModelReflection" in ports.CandidateSource.__annotations__["model_reflection"]
    assert "SkillGapReport" in ports.CandidateSource.__annotations__["skill_gap_report"]
    assert "LearningCandidate" in ports.CandidatePromotionHint.__annotations__["learning_candidate"]


def test_phase8_candidate_ports_do_not_implement_real_capabilities_or_old_chain():
