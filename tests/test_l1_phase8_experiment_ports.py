...
import tiangong_kernel.l1_ports.experiment_ports as ports
...
def test_phase8_experiment_ports_are_abstract():
    assert len(PORT_CLASSES) == 7
    for port_cls in PORT_CLASSES:
        assert inspect.isabstract(port_cls)
        assert port_cls.__name__.endswith("Port")


def test_phase8_experiment_methods_return_result_wrappers():
    for port_cls in PORT_CLASSES:
...
def test_phase8_experiment_data_objects_are_frozen_slots_dataclasses():
    assert DATA_CLASSES
...
def test_phase8_experiment_reuses_l0_and_l1_annotation_names():
    assert "CandidateReference" in ports.ExperimentIntent.__annotations__["candidate"]
    assert "ModelOutcomeAssessment" in ports.ExperimentResult.__annotations__["assessment"]
    assert "ObservationRef" in ports.ExperimentObservation.__annotations__["observation_refs"]


def test_phase8_experiment_ports_do_not_implement_real_capabilities_or_old_chain():
