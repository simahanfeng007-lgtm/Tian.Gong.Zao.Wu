...
import tiangong_kernel.l1_ports.change_ports as ports
...
def test_phase8_change_ports_are_abstract():
...
def test_phase8_change_methods_return_result_wrappers():
    for port_cls in PORT_CLASSES:
...
def test_phase8_change_data_objects_are_frozen_slots_dataclasses():
    assert DATA_CLASSES
...
def test_phase8_change_reuses_l0_and_l1_annotation_names():
    assert "IterationCandidate" in ports.ChangeSetIntent.__annotations__["iteration_candidate"]
    assert "CandidatePromotionHint" in ports.ChangeReviewIntent.__annotations__["candidate_promotion_hint"]
    assert "IterationPatchIntent" in ports.PatchIntentBoundary.__annotations__["patch_intent"]


def test_phase8_change_ports_do_not_implement_real_capabilities_or_old_chain():
