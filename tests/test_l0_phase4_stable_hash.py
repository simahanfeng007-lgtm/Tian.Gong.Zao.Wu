...
def test_phase4_objects_have_stable_hashes():
    objects = (
        RuntimeStateRef(rid(), StateKind.RUNTIME),
        LifecycleRef(rid(), LifecycleState.ACTIVE),
        FailureRef(rid(), FailureKind.PLAN_FAILURE),
        TransactionRef(rid(), TransactionKind.RECOVERY),
        DeletionRef(rid(), DeletionKind.SYSTEM_CLEANUP),
    )
    for obj in objects:
