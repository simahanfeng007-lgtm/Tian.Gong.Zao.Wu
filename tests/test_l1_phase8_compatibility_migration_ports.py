...
import tiangong_kernel.l1_ports.compatibility_migration_ports as ports
...
def test_phase8_compatibility_migration_ports_are_abstract():
...
def test_phase8_compatibility_migration_methods_return_result_wrappers():
    for port_cls in PORT_CLASSES:
...
def test_phase8_compatibility_migration_data_objects_are_frozen_slots_dataclasses():
    assert DATA_CLASSES
...
def test_phase8_compatibility_migration_reuses_l0_and_l1_annotation_names():
    assert "MigrationRef" in ports.MigrationHint.__annotations__["migration_ref"]
    assert "DeprecationRef" in ports.DeprecationNotice.__annotations__["deprecation_ref"]
    assert "SchemaRef" in ports.SchemaMigrationBoundary.__annotations__["schema_ref"]
    assert "VersionRef" in ports.VersionCompatibility.__annotations__["version_ref"]


def test_phase8_compatibility_migration_ports_do_not_implement_real_capabilities_or_old_chain():
