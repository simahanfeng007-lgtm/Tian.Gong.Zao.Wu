...
import tiangong_kernel.l1_ports.validation_ports as ports
...
def test_phase8_validation_ports_are_abstract():
    assert len(PORT_CLASSES) == 13
    for port_cls in PORT_CLASSES:
        assert inspect.isabstract(port_cls)
        assert port_cls.__name__.endswith("Port")


def test_phase8_validation_methods_return_result_wrappers():
    for port_cls in PORT_CLASSES:
...
def test_phase8_validation_data_objects_are_frozen_slots_dataclasses():
    assert DATA_CLASSES
...
def test_phase8_validation_reuses_l0_and_l1_annotation_names():
    assert "ValidationRef" in ports.ValidationReferenceRequest.__annotations__["validation_ref"]
    assert "LearningEvidence" in ports.LearningTestRequest.__annotations__["learning_evidence"]
    assert "IterationCandidate" in ports.IterationVerificationRequest.__annotations__["candidate"]
    assert "EvolutionCandidate" in ports.EvolutionValidationRequest.__annotations__["candidate"]


def test_phase8_validation_ports_do_not_implement_real_capabilities_or_old_chain():
