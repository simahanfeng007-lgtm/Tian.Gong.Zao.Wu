...
FE01 STEP68 / L6.73.8 keeps this Python launcher as the single source of truth
...
def _public_report_path(path: Path) -> str:
    try:
        report_root = reports_dir(ROOT).resolve()
        resolved = path.resolve()
        rel = resolved.relative_to(report_root)
        return "<user-reports>/" + rel.as_posix()
    except Exception:
        return "<user-reports>/desktop_startup_failure_l6710.txt"
...
def _write_startup_failure_report(message: str) -> None:
    try:
        ...
        print(f"启动失败报告：{_public_report_path(report)}")
    except Exception:
        return
...
def _build_env() -> dict[str, str]:
    ...
    if platform_name() == "Darwin":
        ...
        env.setdefault("LINYUANZHE_DESKTOP_PLATFORM", "macos")
        env.setdefault("LINYUANZHE_TK_SCALING", "1.0")
...
def _run_self_check(require_display: bool = False) -> int:
    """Run launcher preflight without overstating GUI display availability."""
    ...
    add("platform", is_supported_desktop_platform(), platform_name())
    add("python_version", sys.version_info >= (3, 10), sys.version.split()[0])
    add("project_root", (ROOT / "frontend" / "linyuanzhe_frontend" / "app.py").exists(), str(ROOT))
    add("bridge_entry", BRIDGE.exists(), str(BRIDGE.relative_to(ROOT)) if BRIDGE.exists() else str(BRIDGE))
    add("provider_config_path", True, str(provider_config_path()))
    import_error = tk_preflight_message(require_display=False)
    add("tkinter_import", not import_error, "ok" if not import_error else import_error.splitlines()[-1])
    if require_display:
        display_error = tk_preflight_message(require_display=True)
        add("tkinter_display", not display_error, "Tk root created" if not display_error else display_error.splitlines()[-1])
    else:
        add("tkinter_display", None, "未验证 DISPLAY；如需桌面显示验收请运行 --self-check-display 或 xvfb-run。")
    max_name = max(len(item[0]) for item in checks)
    failed = False
    for name, status, message in checks:
        failed = failed or status == "FAIL"
        print(f"{name:<{max_name}} : {status} · {message}")
    if failed and require_display:
        _write_startup_failure_report(tk_preflight_message(require_display=True))
    return 1 if failed else 0

def main(argv: Optional[list[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="临渊者桌面端三端通用启动器 FE01 STEP68 / L6.73.8")
    ...
    parser.add_argument("--backend-timeout", type=float, default=float(os.environ.get("LINYUANZHE_BACKEND_TIMEOUT", "900") or 900))
    parser.add_argument("--startup-timeout", type=float, default=float(os.environ.get("LINYUANZHE_STARTUP_TIMEOUT", "45") or 45))
    ...
    env = _build_env()
    env["LINYUANZHE_DESKTOP_ROOT"] = str(ROOT)
    env["LINYUANZHE_ROOT_HINT"] = str(ROOT)
    env["LINYUANZHE_BRIDGE_BACKEND_MODE"] = str(args.backend_mode)
    env["LINYUANZHE_BACKEND_TIMEOUT"] = str(args.backend_timeout)
    bridge_proc, runtime_url = _start_bridge(args, env)
    print(f"本地桥接后端已启动：{runtime_url}", flush=True)
    print("提示：这是桌面一体包的本地桥接；未配置接口时会提示到设置页填写服务地址与接口密钥。", flush=True)
    ...
    try:
        frontend_cmd = [sys.executable, "-S", "-B", "-u", "-m", "linyuanzhe_frontend.app", "--runtime-url", runtime_url]
