...
_CASUAL_CHAT_EXACT = {
    ...
    "忙呢", "忙吗", "忙嘛", "忙不忙", "你忙吗", "你忙嘛", "你忙呢", "在忙吗", "在忙嘛", "还忙吗",
    "在干嘛", "在干啥", "干嘛呢", "干啥呢", "你在干嘛", "你在干啥", "忙啥呢", "忙什么呢",
...
_CASUAL_CHAT_PATTERNS = (
    ...
    re.compile(r"^(你|还)?(在)?忙(呢|吗|嘛|么|不忙)?[?？。！!]*$", re.IGNORECASE),
    re.compile(r"^(你)?在(干嘛|干啥)(呢)?[?？。！!]*$", re.IGNORECASE),
    re.compile(r"^(干嘛|干啥)(呢)?[?？。！!]*$", re.IGNORECASE),
    re.compile(r"^忙(啥|什么)(呢)?[?？。！!]*$", re.IGNORECASE),
    re.compile(r"^(刚刚|刚才)?(什么情况|啥情况|咋回事|怎么回事)[啊呀呢嘛么]?[?？。！!]*$", re.IGNORECASE),
)


def _compact_dialogue_text(value: Any) -> str:
...
def _is_casual_chat_message(value: Any) -> bool:
    ...
    if not text:
        return False
    compact = _compact_dialogue_text(text)
    if not compact:
        return False
...
_DIALOGUE_ONLY_PATTERNS = (
    ...
    re.compile(r"^(为什么|为啥|咋|怎么会|怎么)(会)?(这样|这样子|这样啊).{0,80}[?？。！!]*$", re.IGNORECASE),
    re.compile(r"^.{0,40}(为什么|为啥|咋回事|怎么回事|什么情况|啥情况).{0,60}[?？。！!]*$", re.IGNORECASE),
    re.compile(r"^.{0,40}(是什么问题|是什么原因|什么意思|啥意思|哪里错了|哪里不对|正常吗).{0,50}[?？。！!]*$", re.IGNORECASE),
    re.compile(r"^.{0,40}(是不是|是否|有没有|能不能|能否|可以吗|可不可以).{0,80}[?？。！!]*$", re.IGNORECASE),
    re.compile(r"^(帮我|请|麻烦)?(解释|说明|讲讲|说说|分析)(一下)?(这个|这段|上面|刚才|刚刚)?.{0,60}(报错|错误|回复|现象|问题|截图|提示|原因)?[?？。！!]*$", re.IGNORECASE),
    re.compile(r"^.{0,40}(无法|不能|不会|没法).{0,60}(吗|嘛|么|呢|是不是|是否|为什么|怎么回事|原因)[?？。！!]*$", re.IGNORECASE),
)
_HOST_PATH_RE = re.compile(r"(?:[A-Za-z]:\\|/(?:Users|home|mnt|Volumes|var|tmp|opt|etc|workspace)/)", re.IGNORECASE)
...
_EXECUTION_REQUEST_PATTERNS = (
    re.compile(r"(请|帮我|麻烦|替我|给我|开始|直接|马上|继续|现在)(检查|扫描|质检|修复|修改|改一下|改成|重构|优化|补丁|写入|创建|新建|生成|删除|移除|重命名|移动|复制|读取|打开|运行|执行|测试|复测|验证|打包|压缩|解压|安装|部署|发布|回滚|接入|配置|连接)", re.IGNORECASE),
    re.compile(r"^(检查|扫描|质检|修复|修改|改一下|改成|重构|优化|补丁|写入|创建|新建|生成|删除|移除|重命名|移动|复制|读取|打开|运行|执行|测试|复测|验证|打包|压缩|解压|安装|部署|发布|回滚|接入|配置|连接)", re.IGNORECASE),
    re.compile(r"(定位\s*bug|修\s*bug|fix\s+bug|repair|patch|hotfix|smoke|verifier|回归|fresh\s*extract)", re.IGNORECASE),
    re.compile(r"(跑一下|跑一遍|执行一下|启动一下|测一下|复测一下|打包一下|修一下|改一下|写一个|建一个|生成一个)", re.IGNORECASE),
    re.compile(r"(把|将).{0,40}(写入|改成|移动|复制|删除|打包|压缩|解压|发布|部署|上传|导出)", re.IGNORECASE),
)


def _looks_like_execution_request(value: Any) -> bool:
    text = str(value or "").strip()
    compact = _compact_dialogue_text(text)
    lowered = text.lower()
    if not compact:
        return False
    if any(pattern.search(compact) for pattern in _EXECUTION_REQUEST_PATTERNS):
        return True
    target_present = bool(_HOST_PATH_RE.search(text) or _FILE_TARGET_RE.search(text) or _CODE_FENCE_RE.search(text))
...
def _is_dialogue_only_message(value: Any) -> bool:
    ...
    if _is_casual_chat_message(text):
        return True
    if _looks_like_execution_request(text):
        return False
    if compact in _DIALOGUE_ONLY_EXACT or compact.lower() in {x.lower() for x in _DIALOGUE_ONLY_EXACT}:
        return True
    if len(compact) <= 140 and any(pattern.search(compact) for pattern in _DIALOGUE_ONLY_PATTERNS):
        return True
    if len(compact) <= 90 and (("？" in text) or ("?" in text)):
...
def _local_dialogue_answer(value: Any, state: "BridgeState" | None = None) -> str:
    ...
    compact = _compact_dialogue_text(text)
    if _is_casual_chat_message(text):
        if any(marker in compact for marker in ("忙", "干嘛", "干啥")):
            return "在，不忙。你说。"
        return "在。"
    persona = _safe_text(getattr(state, "persona_name", "临渊者") if state is not None else "临渊者", 32) or "临渊者"
    if any(marker in compact for marker in ("为什么", "为啥", "怎么回事", "什么情况", "错误", "报错", "回复这个", "是什么问题")):
        return (
            "这类问题我会先按普通分析来答，不应直接进入工具链。"
            "只有你明确要求读取文件、修复、运行、打包或接入外部系统时，我才会切到工作执行。"
        )
    if any(marker in compact for marker in ("长链", "网关", "微信", "飞书", "能不能", "有没有", "是不是")):
        return (
            "这是普通能力/状态提问，我会按聊天解释；不会因为左下角停在“工作”就强行启动 Runtime 工具。"
            "需要真实执行时，请明确说要读取、写入、修复、运行、打包或接入。"
        )
...
def _runtime_directives_from_payload(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    # Q22: a dialogue/explanation question is still chat even when a stale
    # frontend payload claims activation/tools.  This protects provider-backed
    # Windows desktop sessions from surfacing "主脑未激活工具链" for messages such
    # as "为什么会这样" / "这个错误是什么意思" / "是不是还没有网关".
    dialogue_override = _is_dialogue_only_message(user_message)
    if dialogue_override:
        task_mode, planner_mode, output_contract = WORK_MODE_TO_RUNTIME["chat"]
        return {
            ...
            "file_intent": False,
            "code_intent": False,
            "casual_chat_override": True,
            "dialogue_only_override": True,
            "dialogue_analysis_override": bool(not _is_casual_chat_message(user_message)),
    ...
    return {
        ...
        "dialogue_only_override": False,
        "dialogue_analysis_override": False,
        "llm_fills_activation_form": True,
        "user_message_digest": _digest(str(user_message)),
    }
...
def _run_backend_once(message: str, state: BridgeState, *, runtime_directives: Mapping[str, Any] | None = None, run_id: str = "") -> tuple[str, int, str]:
    ...
    if returncode != 0 and _is_dialogue_only_message(message):
        ...
        if any(marker in diagnostic for marker in ("activationform", "activation_chat_or_no_tools", "unicodeencodeerror", "provider", "api", "模型接口", "请求编码", "工作模式未执行", "主脑本轮未激活工具链", "工具任务失败", "runtime 工具任务失败", "model_required", "retry_activation_with_work_contract")):
            state.last_bridge_error_kind = ""
