...
def reports_dir(root: Path | None = None) -> Path:
    """Writable diagnostics directory outside the shipped package by default."""
    ...
    return user_config_dir() / "reports"
...
def tk_preflight_message(*, require_display: bool = True) -> str:
    ...
    try:
        ...
        return (
            ...
            f"TCL_LIBRARY={'<set>' if os.environ.get('TCL_LIBRARY') else '<unset>'}\n"
            f"TK_LIBRARY={'<set>' if os.environ.get('TK_LIBRARY') else '<unset>'}\n"
            f"Error={exc}\n"
