    ...
    def evaluate(
        ...
        *,
            ...
            if status == "blocked":
                ...
                issues.append(
                    QualityGateIssue(
                        ...
                        severity="P1",
                        message=f"质量检查 {name} 失败，当前不允许发布打包。",
        ...
        if severity_counts.get("P0", 0) > 0:
            ...
            summary = "质量门阻断：存在 P0 或治理阻断项，必须先修正边界/命令/权限。"
        elif severity_counts.get("P1", 0) > 0:
            decision = "fail"
            allow_package = False
            allow_continue = True
            summary = "质量门失败：存在 P1 或测试失败项，允许继续修复但不允许发布打包。"
        elif severity_counts.get("P2", 0) > 0:
            decision = "warn"
            allow_package = True
            allow_continue = True
            summary = "质量门警告：无阻断失败，但存在 P2 警告，允许受控打包并需在报告中披露。"
        else:
            decision = "pass"
            allow_package = True
            allow_continue = True
            summary = "质量门通过：基础质量检查与诊断摘要未发现阻断项。"
