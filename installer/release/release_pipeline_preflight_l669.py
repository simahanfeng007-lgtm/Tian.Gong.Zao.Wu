...
def main() -> int:
    ...
    runs = [
        _run("startup_self_check_l669", [sys.executable, str(ROOT / "installer" / "startup" / "startup_self_check_l669.py")]),
        _run("version_slot_validation_l669", [sys.executable, str(ROOT / "installer" / "build" / "version_slot_validate_l669.py")]),
        _run("windows_packager_dry_run_l669", [sys.executable, str(ROOT / "installer" / "build" / "package_builder_dry_run_l669.py")]),
    ]
    manifest_checks = {
    ...
    print(json.dumps({"ok": ok, "ready_for_combine": False, "report": str(out)}, ensure_ascii=False, indent=2))
