...
def main(argv: list[str] | None = None) -> int:
    ...
    events = [
        RuntimeSseEvent(event="run_started", seq=1, run_id="run-l662", task_id="task-l662", payload={"status": "running", "latency_ms": 12}),
        RuntimeSseEvent(event="planner_started", seq=2, run_id="run-l662", task_id="task-l662", payload={"planner_mode": "unified"}),
        RuntimeSseEvent(event="planner_plan", seq=3, run_id="run-l662", task_id="task-l662", payload={"steps": [{"tool_name": "readonly_probe", "status": "queued"}]}),
        RuntimeSseEvent(event="quality_gate", seq=4, run_id="run-l662", task_id="task-l662", payload={"risk_level": "A5", "decision": "confirmation_required", "gate_id": "gate-l662", "audit_ref": "audit-l662", "action_summary": "只读观测测试"}),
        RuntimeSseEvent(event="tool_started", seq=5, run_id="run-l662", task_id="task-l662", payload={"tool_name": "readonly_probe"}),
        RuntimeSseEvent(event="tool_result", seq=6, run_id="run-l662", task_id="task-l662", payload={"tool_name": "readonly_probe", "status": "ok", "audit_ref": "audit-l662", "output_summary": "只读探测完成"}),
        RuntimeSseEvent(event="audit_event", seq=7, run_id="run-l662", task_id="task-l662", payload={"audit_id": "audit-l662", "message": "审计只读投影"}),
        RuntimeSseEvent(event="runtime_state", seq=8, run_id="run-l662", task_id="task-l662", payload={"status": "RUNNING", "provider_base_url_configured": True, "provider_base_url_digest": "digest_example_runtime", "authorization_configured": True, "authorization_digest": "digest_token_marker"}),
        RuntimeSseEvent(event="assistant_final", seq=9, run_id="run-l662", task_id="task-l662", payload={"status": "ok", "content": "观测台 smoke 完成"}),
        RuntimeSseEvent(event="run_terminal", seq=10, run_id="run-l662", task_id="task-l662", payload={"status": "ok"}),
    ]
    for event in events:
