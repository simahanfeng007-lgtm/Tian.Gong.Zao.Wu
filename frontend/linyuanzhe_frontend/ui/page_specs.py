...
PAGE_DEFINITIONS: Tuple[PageSpec, ...] = (
    ...
    PageSpec("history", "历史", "", "本地对话历史、只读回放、全文检索与导出"),
    PageSpec("memory", "记忆", "", "脱敏记忆摘要、摘要指纹、证据引用、文件/工作区入口"),
    PageSpec("four_paths", "系统", "", "执行、记忆、情志、生命周期、连接、迭代、安装与 数据更新入口"),
    PageSpec("settings", "设置", "", "模型服务向导、主模型、外观与运行时只读接入配置"),
)

EXTRA_PAGE_DEFINITIONS: Tuple[PageSpec, ...] = (
    PageSpec("execution", "执行详情", "", "执行链、质量门、恢复续接、确认票据摘要"),
    PageSpec("observability", "运行观测", "", "轨迹、事件、预算、质量门、错误与流式事件收口顺序，只读展示"),
    PageSpec("files", "文件交接", "", "文件传输请求、附件交接、下载/上传回执与边界说明"),
    PageSpec("workspace", "工作区", "", "工作区、沙箱边界、目录白名单、文件授权与下载中转回执"),
    ...
    PageSpec("hooks", "规则", "", "规则总线、阻断记录、请求守卫与事件守卫，只读展示"),
)
