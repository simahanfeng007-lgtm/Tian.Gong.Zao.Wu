...
它只描述前端显示，不参与 运行时执行，也不授予任何工具/模型权限。
"""
...
DESKTOP_POLISH_REQUIREMENTS = (
    ...
    "首页必须能直接输入和发送消息",
    "二级详情渐进披露",
    "模型服务设置页以向导承载 服务商、服务地址、接口密钥、主模型和检查配置",
    "模型服务设置只向 模型服务设置端点 提交写入请求，回执只展示 已配置/摘要指纹/错误态",
    "正式合成前必须完成真实 运行时实例预检；契约服务器回归不能替代真实联调",
    "质量门 行动守卫卡只能提交确认请求，不能前端放行",
    "审计/回滚卡片只读展示，不能前端写审计或应用回滚",
    "前端仍禁止裸调 模型服务开发包、适配器、工具、记忆、审计、回滚",
    "视觉色彩从 Design Tokens 派生，避免散落硬编码扩大",
    "联调与 RC 预检报告只写 运行时地址摘要指纹，不写明文地址或 模型服务凭证",
