...
class FrontendActionsMixin:
    ...
    def _resolve_submit_work_mode_payload(self, text: str) -> Dict[str, Any]:
        ...
        if payload.get("dialogue_analysis_override"):
            self.stream_status_var.set("工作模式：检测到普通问题/分析问题，已按聊天提交，不触发 Planner 或工具链。")
        elif payload.get("casual_chat_override"):
            self.stream_status_var.set("工作模式：检测到寒暄/普通提问，已按聊天提交，不触发 Planner 或工具链。")
        elif payload.get("mode") == "work":
            self.stream_status_var.set("工作模式：已提交；Runtime 将先让模型填写 ActivationForm，再决定是否进入工具链。")
        else:
    ...
    def _send_message(self) -> None:
        ...
        status = "流式状态：工作模式提交中 · 等待 Runtime/ActivationForm 判定" if work_payload.get("mode") == "work" else "流式状态：聊天提交中"
