...
DEFAULT_REPORT = Path(tempfile.mkdtemp(prefix="linyuanzhe_smoke_report_")) / "l6738_real_runtime_e2e_smoke_report.json"
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="FE01 STEP68 / L6.73.8 frontend/backend E2E smoke")
    ...
    try:
        ...
        report_label = f"<tmp>/{out_path.name}" if str(out_path).startswith(tempfile.gettempdir()) else out_path.name
        print(json.dumps({"ok": payload["ok"], "mode": mode, "report": report_label}, ensure_ascii=False, indent=2))
