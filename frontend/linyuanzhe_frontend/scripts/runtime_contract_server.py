...
class RuntimeContractHandler(BaseHTTPRequestHandler):
    """Local controlled Runtime contract server for L6.73.8 desktop E2E/provider settings/RC preflight smoke.
    ...
    server_version = "LinyuanzheRuntimeContractL6738/1.0"
    ...
    def do_GET(self) -> None:  # noqa: N802 - stdlib handler API
            ...
            self._send_json(
                {
                    ...
                    "current_stage": "L6.73.8 Runtime contract server ready",
                    "status_bar": {
                        "runtime_status": "active",
        ...
        if self.path == PRODUCT_METADATA_ENDPOINT:
            self._send_json(
                {
                    "schema": "tiangong.l6_73_8.product_identity.v1",
                    "product_name": "天工造物 / 临渊者",
                    "unique_developer": "于泳翔",
                    "angel_investor": "胖胖龙",
                    "frontend_full_version": "FE01 STEP68 / L6.73.8",
                    "frontend_runtime_version": "L6.73.8",
                    "provider_config_schema_version": "tiangong.l6_73_8.local_provider_config.v1",
        ...
        if self.path == INSTALLER_MANIFEST_ENDPOINT:
            slots = [
                VersionSlotProjection(slot_name="active", version_label="FE01 STEP68 / L6.73.8", state="active", path_digest="SLOT-ACTIVE-CONTRACT", rollback_capable=True, message="contract server active slot"),
            ...
            self._send_json({"installer_rc_contract": "tiangong.l6_73_8.installer_rc.v1", "installer_manifest": manifest.to_dict(), "version_slots": [item.to_dict() for item in slots], "startup_self_checks": [item.to_dict() for item in startup_checks], "installer_last_message": "安装器 RC 投影已脱敏读取。", "policy": installer_rc_policy()})
            return
