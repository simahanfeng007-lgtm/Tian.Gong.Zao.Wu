...
def _rule_file_transfer_request_only(context: Mapping[str, Any]) -> HookDecision:
    ...
    raw_path_fields = ("runtime_handoff_path", "local_path", "source_path", "original_path", "raw_path")
    for field in raw_path_fields:
        raw_value = str(payload.get(field, "") or "").strip()
        if raw_value:
            return HookDecision.block("file_transfer_no_raw_path", f"file transfer public payload may not contain raw local path field: {field}")
...
class HookBus:
    ...
    def __init__(self, rules: Sequence[HookRule]) -> None:
        ...
        self._seq = 0
        # Q18: 高频 SSE 事件下 HookBus 投影不能每次用 dataclasses.asdict
        # 深拷贝全部记录。缓存统计与导出摘要；新增记录时失效，连续 projection
        # 同步时复用缓存，避免 500 个工具事件把前端卡住。
        self._cached_stats: HookStats | None = None
        self._cached_export_digest_full: str = ""
    ...
    def _append_record(self, stage: str, decision: HookDecision, context: Mapping[str, Any]) -> None:
        ...
        self._cached_export_digest_full = ""

    def stats(self) -> HookStats:
        if self._cached_stats is None:
            self._cached_stats = HookStats.from_records(self.records)
        return self._cached_stats

    def export_digest(self, length: int = 16) -> str:
        if not self._cached_export_digest_full:
            # Keep the digest deterministic while avoiding asdict's recursive deep
            # copy cost on every projection sync.  The exported digest is a public
            # change marker, not a raw record export.
            data = "|".join(
                "\x1f".join(
                    (
                        str(record.seq),
                        ...
                        record.severity,
                        record.source_event,
                        record.run_id_digest,
                        record.task_id_digest,
                        record.ticket_id_digest,
                        record.timestamp,
                    )
                )
                for record in self.records
            )
            self._cached_export_digest_full = sha256(data.encode("utf-8", errors="ignore")).hexdigest()
        return self._cached_export_digest_full[:length]
