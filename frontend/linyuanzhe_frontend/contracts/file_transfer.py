...
"""L6.73.8 file transfer request contract.

The desktop frontend prepares a sanitized public request for Runtime / RC
validation, but raw host paths may not appear in public payloads, reports,
RuntimeSnapshot, HookBus records, or SSE projections.

Two surfaces are intentionally separated:

1. ``to_payload()`` / public request payload
   - no raw local path;
   - carries file metadata, sha256, local_path_digest and handoff_token only.
2. ``to_bridge_payload()`` / ``to_private_runtime_headers()``
   - local desktop bridge private channel only;
   - carries ``runtime_handoff_path`` so the localhost bridge can copy the file
     into the governed handoff directory.
"""
...
FILE_TRANSFER_CONTRACT_VERSION = "tiangong.l6_73_8.file_transfer_request.v2"
    ...
    def from_path(
        ...
        *,
        ...
        if size > MAX_FILE_TRANSFER_BYTES:
            raise ValueError("selected file exceeds L6.73.8 transfer size limit")
        resolved = path.resolve()
        mime, _encoding = mimetypes.guess_type(path.name)
        sha = _sha256_file(path)
        local_digest = digest_text(str(resolved), 16)
        token = digest_text(f"{resolved}|{sha}|{size}", 24)
        return cls(
            ...
            local_path_digest=local_digest,
            handoff_token=token,
            runtime_handoff_path=str(resolved),
        )

    def to_payload(self) -> Dict[str, Any]:
        """Return public / RC-safe request payload without raw host paths."""
        return {
            ...
            "local_path_digest": self.local_path_digest,
            "handoff_token": self.handoff_token,
            "frontend_contract": self.frontend_contract,
            "route_to_runtime_only": self.route_to_runtime_only,
            "no_frontend_tool_execution": self.no_frontend_tool_execution,
            "no_frontend_memory_write": self.no_frontend_memory_write,
            "no_frontend_audit_write": self.no_frontend_audit_write,
            "no_frontend_rollback_apply": self.no_frontend_rollback_apply,
            "no_frontend_path_exposure": self.no_frontend_path_exposure,
            "raw_content_inline": self.raw_content_inline,
        }

    def to_bridge_payload(self) -> Dict[str, Any]:
        payload = self.to_payload()
        payload["runtime_handoff_path"] = self.runtime_handoff_path
        payload["private_local_bridge_only"] = True
        return payload

    def to_private_runtime_headers(self) -> Dict[str, str]:
        if not self.runtime_handoff_path:
            return {}
        encoded_path = base64.urlsafe_b64encode(self.runtime_handoff_path.encode("utf-8", errors="surrogatepass")).decode("ascii")
        return {
            "X-Linyuanzhe-Local-Handoff-Path-B64": encoded_path,
            "X-Linyuanzhe-Local-Handoff-Token": self.handoff_token,
        }
