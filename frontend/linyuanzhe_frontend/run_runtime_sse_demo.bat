...
echo [Linyuanzhe] Starting FE.01 Runtime SSE mode: %LINYUANZHE_RUNTIME_URL%
call :find_python_tk
if errorlevel 1 (
  pause
  exit /b 1
)
"%PYTHON_EXE%" -S -B run_runtime_sse_demo.py
set "RC=%ERRORLEVEL%"
if not "%RC%"=="0" (
  echo [Linyuanzhe] Launch failed. Check Runtime URL and tkinter desktop support.
  pause
)
exit /b %RC%
