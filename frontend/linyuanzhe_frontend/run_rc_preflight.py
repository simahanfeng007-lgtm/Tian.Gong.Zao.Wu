...
from linyuanzhe_frontend.version_info import FE_FULL_VERSION, FE_RUNTIME_VERSION, FE_STEP, PROVIDER_CONFIG_SCHEMA_VERSION

ROOT = Path(__file__).resolve().parent
DEFAULT_REPORT = Path(__import__("tempfile").mkdtemp(prefix="linyuanzhe_rc_l6738_")) / "l6738_rc_preflight_report.json"
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="FE01 STEP68 / L6.73.8 frontend/backend RC preflight")
    ...
    try:
        ...
        payload["current_package_identity"] = {
            "expected_fe_full_version": "FE01 STEP68 / L6.73.8",
            "actual_fe_full_version": FE_FULL_VERSION,
            "fe_step": FE_STEP,
            "runtime_version": FE_RUNTIME_VERSION,
            "provider_config_schema_version": PROVIDER_CONFIG_SCHEMA_VERSION,
            "ok": FE_FULL_VERSION == "FE01 STEP68 / L6.73.8" and FE_RUNTIME_VERSION == "L6.73.8",
        }
        payload["note"] = "本报告不包含 Runtime URL 明文、Provider Key 明文或 Provider Base URL 明文。"
        if not payload["current_package_identity"]["ok"]:
            payload["ok"] = False
            payload.setdefault("merge_blockers", []).append("current_package_identity_mismatch")
        _write_report(Path(args.out), payload)
        print(json.dumps({"ok": payload["ok"], "ready_for_combine": payload["ready_for_combine"], "rc_status": payload["rc_status"], "mode": mode, "report": _public_report_path(Path(args.out))}, ensure_ascii=False, indent=2))
