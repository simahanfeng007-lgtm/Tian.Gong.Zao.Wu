...
"""DataUp safe updater core for Linyuanzhe FE01 STEP68 / L6.73.8.
...
from network_policy_l67218 import NetworkPolicyError, urlopen_with_policy
...
SIGNATURE_FILENAMES = (
    "dataup_manifest.sig",
    "dataup_signature.sig",
    "manifest.sig",
)
DATAUP_PUBLIC_KEY_ENV = "LINYUANZHE_DATAUP_PUBLIC_KEY"
DATAUP_PUBLIC_KEY_DEFAULT = Path("dataup") / "public_keys" / "dataup_ed25519_public_key.pem"
...
SENSITIVE_PATTERNS = (
    re.compile(r"(?i)mockkey_[A-Za-z0-9_\-]{12,}"),
...
class PackagePlan:
    ...
    signature_note: str = "signature verification required"
...
def _load_manifest(zf: zipfile.ZipFile, member: str) -> tuple[dict[str, Any], bytes]:
    try:
        raw = zf.read(member)
...
def _public_key_path(root: Path) -> Path:
    env = os.environ.get(DATAUP_PUBLIC_KEY_ENV, "").strip()
    if env:
        return Path(env).expanduser().resolve()
    return (root / DATAUP_PUBLIC_KEY_DEFAULT).resolve()


def _find_signature_member(zf: zipfile.ZipFile, manifest_member: str) -> str:
    names = [n for n in zf.namelist() if not n.endswith("/")]
    manifest_parent = str(PurePosixPath(manifest_member).parent)
    candidates: list[str] = []
    for filename in SIGNATURE_FILENAMES:
        if manifest_parent != ".":
            candidates.append(str(PurePosixPath(manifest_parent) / filename))
...
def _decode_signature(raw: bytes) -> bytes:
    ...
    if not stripped:
        return b""
    # Allow raw detached signature, lowercase/uppercase hex, or one-line base64.
    try:
...
def _openssl_verify_ed25519(public_key: Path, manifest_raw: bytes, signature_raw: bytes) -> tuple[bool, str]:
    openssl = shutil.which("openssl")
    if not openssl:
        return False, "openssl unavailable for Ed25519 verification"
    with tempfile.TemporaryDirectory(prefix="linyuanzhe_dataup_sig_") as td:
        tmp = Path(td)
        ...
        proc = subprocess.run(
            [openssl, "pkeyutl", "-verify", "-pubin", "-inkey", str(public_key), "-rawin", "-in", str(manifest_path), "-sigfile", str(sig_path)],
            text=True,
            capture_output=True,
            timeout=30,
        )
    if proc.returncode == 0:
        return True, "Ed25519 manifest signature verified"
    return False, "Ed25519 verification failed: " + _safe_text((proc.stderr or proc.stdout or "openssl verify failed"), 240)


def _verify_manifest_signature(zf: zipfile.ZipFile, manifest_member: str, manifest_raw: bytes, root: Path) -> tuple[bool, str]:
    sig_member = _find_signature_member(zf, manifest_member)
    if not sig_member:
        return False, "missing detached manifest signature"
    signature_raw = _decode_signature(zf.read(sig_member))
    if not signature_raw:
        return False, "empty manifest signature"
    public_key = _public_key_path(root)
    if not public_key.exists():
        return False, f"public key missing: {_safe_text(public_key)}"
    return _openssl_verify_ed25519(public_key, manifest_raw, signature_raw)
...
def plan_package(package_path: Path, root: Path, *, source_kind: str = "local_package") -> PackagePlan:
    ...
    with zipfile.ZipFile(package_path, "r") as zf:
        ...
        manifest, manifest_raw = _load_manifest(zf, manifest_member)
        ...
        signature_verified, signature_note = _verify_manifest_signature(zf, manifest_member, manifest_raw, root)
        if not signature_verified:
            blocked.append("signature verification failed: " + signature_note)
...
def _default_report_dir() -> Path:
    return Path(os.environ.get("LINYUANZHE_REPORT_DIR") or tempfile.mkdtemp(prefix="linyuanzhe_dataup_"))
...
def _download_json(url: str, timeout: float = 10.0) -> dict[str, Any]:
    if not str(url).lower().startswith("https://"):
        raise DataUpError("DataUp latest metadata URL must use HTTPS")
    req = urllib.request.Request(url, headers={"Accept": "application/json", "User-Agent": "Linyuanzhe-DataUp/0.1"}, method="GET")
    with urlopen_with_policy(req, timeout=timeout, allow_loopback_http=False, purpose="dataup_latest") as resp:
...
def download_package_from_latest(latest_result: Mapping[str, Any], dest_dir: Path) -> Path:
    ...
    if not package_url.lower().startswith("https://"):
        raise DataUpError("DataUp package URL must use HTTPS")
    ...
    with urlopen_with_policy(req, timeout=30, allow_loopback_http=False, purpose="dataup_package") as resp:
        ...
        if args.check or package is None:
            ...
            if args.check and not args.apply and not args.dry_run and package is None:
                if latest_result.get("ok"):
                    ...
                    payload.update({
                        "ok": True,
                        "stage": "offline_latest_unavailable",
                        "update_available": False,
                        "message": "Online DataUp latest metadata is unavailable; keeping the offline package unchanged. Retry online later or pass --package for a local DataUp package.",
                    })
                out = _write_report(root, "dataup_last_update_report_l6717.json", payload, report_path)
                payload["report"] = _public_report_path(out)
        ...
        if package is None:
            if latest_result is None or not latest_result.get("ok"):
                payload.update({
                    ...
                    "stage": "offline_latest_unavailable",
                    "update_available": False,
                    "message": "未取得在线 DataUp 最新元数据；当前离线包保持不变，可稍后联网重试或使用 --package 指定本地 DataUp 包。",
                })
                out = _write_report(root, "dataup_last_update_report_l6717.json", payload, report_path)
                payload["report"] = _public_report_path(out)
                print(_json_dumps(payload))
