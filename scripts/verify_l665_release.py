...
"""L6.65 aggregate release evidence checker.

This verifier intentionally consumes already generated evidence files rather than
spawning the contract server again. The standalone preflight scripts remain the
source of truth for each check. This avoids a known sandbox issue where local
contract-server children can keep file descriptors open and block an aggregate
verifier process even after the preflight itself has passed.
"""
...
def main() -> int:
    ...
    results: list[dict[str, Any]] = [
        _exit_result("l665_backend_compileall"),
        _exit_result("l665_frontend_scripts_compileall"),
        _exit_result("l665_observability_preflight"),
        _exit_result("l665_hookbus_preflight"),
        _exit_result("l665_file_transfer_interrupt_preflight"),
        _exit_result("l665_workspace_preflight"),
        _json_ok("l665_rc_preflight_contract_server", REPORTS / "rc_preflight_l665_contract_server.json"),
        _scan_ok("l665_scan", REPORTS / "scan_l659.json"),
    ]
    if runtime_present:
        results.append(_json_ok("l665_real_runtime_unlock", REPORTS / "real_runtime_unlock_l665_verify.json", "ready_for_combine"))
    else:
        results.append(_exit_result("l665_real_runtime_unlock_absent_expected", {2}))
    summary: dict[str, Any] = {
        "contract_version": "tiangong.l6_65.release_verify.v2",
        ...
        "note": "L6.65 adds Agent Workspace / sandbox and file authorization display/request boundaries. Aggregate verifier consumes generated evidence; standalone scripts rerun each preflight.",
    }
    if runtime_present and (REPORTS / "real_runtime_unlock_l665_verify.json").exists():
        try:
            payload = json.loads((REPORTS / "real_runtime_unlock_l665_verify.json").read_text(encoding="utf-8"))
            summary["ready_for_combine"] = bool(payload.get("ready_for_combine"))
        except Exception as exc:
            summary["real_unlock_report_read_error"] = str(exc)[:160]
    out = REPORTS / "validation_summary_l665.json"
