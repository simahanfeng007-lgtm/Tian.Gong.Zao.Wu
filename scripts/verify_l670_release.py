...
"""FE01 STEP31 / L6.70 aggregate release evidence checker."""
...
def _json_ok(name: str, path: Path, key: str = "ok", expected: bool = True) -> dict[str, Any]:
    ...
    return {"name": name, "returncode": 0 if ok else 1, "ok": ok, "log": str(path), "key": key, "value": value}
...
def _scan_ok(name: str, path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        return {"name": name, "returncode": 1, "ok": False, "log": str(path), "reason": str(exc)[:160]}
    return {"name": name, "returncode": 0 if ok else 1, "ok": ok, "log": str(path), "counts": counts}
...
def main() -> int:
    ...
    print(json.dumps({"ok": summary["ok"], "ready_for_combine": summary["ready_for_combine"], "blockers": blockers, "report": str(out)}, ensure_ascii=False, indent=2))
