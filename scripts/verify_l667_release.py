...
"""L6.67 aggregate release evidence checker."""
...
def main() -> int:
    ...
    results: list[dict[str, Any]] = [
        _exit_result("l667_backend_compileall"),
        _exit_result("l667_frontend_scripts_compileall"),
        _exit_result("l667_observability_preflight"),
        _exit_result("l667_hookbus_preflight"),
        _exit_result("l667_file_transfer_interrupt_preflight"),
        _exit_result("l667_workspace_preflight"),
        _exit_result("l667_connector_registry_preflight"),
        _exit_result("l667_session_manager_preflight"),
        _json_ok("l667_rc_preflight_contract_server", REPORTS / "rc_preflight_l667_contract_server.json"),
        _scan_ok("l667_scan", REPORTS / "scan_l659.json"),
    ]
    if runtime_present:
        results.append(_json_ok("l667_real_runtime_unlock", REPORTS / "real_runtime_unlock_l667_verify.json", "ready_for_combine"))
    else:
        results.append(_exit_result("l667_real_runtime_unlock_absent_expected", {2}))
    summary: dict[str, Any] = {
        "contract_version": "tiangong.l6_67.release_verify.v1",
        "checked_at": datetime.now().isoformat(timespec="seconds"),
        "runtime_url_present": runtime_present,
        "results": results,
        "ok": all(item["ok"] for item in results),
        "ready_for_combine": False,
        "note": "L6.67 adds multi-task Session Manager. Verifier consumes generated evidence; true combine remains blocked until real Runtime smoke passes.",
    }
    if runtime_present and (REPORTS / "real_runtime_unlock_l667_verify.json").exists():
        try:
            payload = json.loads((REPORTS / "real_runtime_unlock_l667_verify.json").read_text(encoding="utf-8"))
            summary["ready_for_combine"] = bool(payload.get("ready_for_combine"))
        except Exception as exc:
            summary["real_unlock_report_read_error"] = str(exc)[:160]
    out = REPORTS / "validation_summary_l667.json"
