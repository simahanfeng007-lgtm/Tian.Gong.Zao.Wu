...
"""FE01 STEP68 / L6.73.8 compatibility wrapper for historical L6.71 evidence."""
...
def _exit_ok(name: str) -> dict[str, Any]:
    ...
    try:
        ...
        rc = 1
    return {"name": name, "ok": rc == 0, "returncode": rc, "exit": _public_path(p), "log": _public_path(REPORTS / f"{name}.log")}


def main() -> int:
    desktop = _read_json(REPORTS / "desktop_bundle_preflight_l671.json")
    results = [
        {"name": "desktop_bundle_preflight_l671", "ok": bool(desktop.get("ok")), "report": _public_path(REPORTS / "desktop_bundle_preflight_l671.json")},
    ...
    summary = {
        ...
        "note": "Historical desktop visual-click verifier retained for compatibility; it does not replace real Runtime evidence.",
    }
    out = REPORTS / "validation_summary_l671.json"
    out.write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
    print(json.dumps({"ok": ok, "desktop_all_in_one_ready": ok, "ready_for_combine": False, "report": _public_path(out)}, ensure_ascii=False, indent=2))
    return 0
