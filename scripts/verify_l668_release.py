...
"""L6.68 aggregate release evidence checker."""
...
def main() -> int:
    ...
    results: list[dict[str, Any]] = [
        _exit_result("l668_backend_compileall"),
        _exit_result("l668_frontend_scripts_compileall"),
        _exit_result("l668_observability_preflight"),
        _exit_result("l668_hookbus_preflight"),
        _exit_result("l668_file_transfer_interrupt_preflight"),
        _exit_result("l668_workspace_preflight"),
        _exit_result("l668_connector_registry_preflight"),
        _exit_result("l668_session_manager_preflight"),
        _exit_result("l668_installer_rc_preflight"),
        _json_ok("l668_rc_preflight_contract_server", REPORTS / "rc_preflight_l668_contract_server.json"),
        _scan_ok("l668_scan", REPORTS / "scan_l659.json"),
    ]
    if runtime_present:
        results.append(_json_ok("l668_real_runtime_unlock", REPORTS / "real_runtime_unlock_l668_verify.json", "ready_for_combine"))
    else:
        results.append(_exit_result("l668_real_runtime_unlock_absent_expected", {2}))
    summary: dict[str, Any] = {
        "contract_version": "tiangong.l6_68.release_verify.v1",
        "checked_at": datetime.now().isoformat(timespec="seconds"),
        "runtime_url_present": runtime_present,
        "results": results,
        "ok": all(item["ok"] for item in results),
        "ready_for_combine": False,
        "note": "L6.68 adds installer RC pre-stage structure. True combine remains blocked until real Runtime smoke passes.",
    }
    if runtime_present and (REPORTS / "real_runtime_unlock_l668_verify.json").exists():
        try:
            payload = json.loads((REPORTS / "real_runtime_unlock_l668_verify.json").read_text(encoding="utf-8"))
            summary["ready_for_combine"] = bool(payload.get("ready_for_combine"))
        except Exception as exc:
            summary["real_unlock_report_read_error"] = str(exc)[:160]
    out = REPORTS / "validation_summary_l668.json"
